...
struct custom_unique_ptr {
    ...
    custom_unique_ptr(pointer_type p, deleter_type&& d) : mptr_(p), dlt_(std::move(d)) { p=nullptr; }
    custom_unique_ptr(const custom_unique_ptr&) = delete;
    custom_unique_ptr(custom_unique_ptr&& p) : mptr_(std::move(p.mptr_)) {
        p.mptr_=nullptr;
    }
...
int main() {
    ...
    std::cout << "\n" "3) Custom lambda-expression deleter and exception safety demo\n";
    try
    {
